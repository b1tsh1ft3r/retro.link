...
SETUP_RLINK:
    ...
    move.w   #$1,$fffff03c.w      ; HIDE LIFE BARS AND FLASHING START TEXT
    move.w   #$5,$ffffaac0.w      ; SET BACKGROUND TO USE
    jsr      $4142                ; LOAD BACKGROUND GRAPHICS
    jsr      $210a4               ; DMA UP THE BACKGROUND
    ;jsr      $419a               ; PLAY STAGE MUSIC    
                                  ; enabling this causes us to not be able to exit. i think hblank is running and z80 still needs
                                  ; updating which causes a hang and graphics to be garbled.

    move.w  #$10,$ffffaac0        ; force game to reset bg on first go around to normalize things

    jsr      LOAD_FONT            ; This has to be here... sigh...

    MOVE.W  #34,SCREEN_X          ; SET BASE SCREEN XPOS (34)
    MOVE.W  #14,SCREEN_Y          ; SET BASE SCREEN YPOS (0)
...
;****************************************************************************
UPDATE_SCREEN:
    jsr      $21084               ; NEEDED for screen drawing
    jsr      $5470                ; wait for vbl
    jsr      $5878                ; update music
    jsr      $4aea                ; do some dma
    rts
...
ESTABLISH_TXT       DC.B '      ESTABLISHING COMMUNICATION...     ',0
LATENCY_TXT         DC.B '         CALCULATING LATENCY...         ',0
SYNC_TXT            DC.B '            SYNCHRONIZING...            ',0
TIMEOUT_TXT:        DC.B '            NETWORK TIMEOUT             ',0
